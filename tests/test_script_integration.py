...
class TestScriptIntegration(unittest.TestCase):
    ...
    def test_scripts_have_i18n_imports(self):
        ...
        for script_name in self.integrated_scripts:
            with self.subTest(script=script_name):
                ...
                self.assertIn(
                    "from language_selector import get_language", content, f"{script_name} missing language_selector import"
                )
                self.assertIn("from loader import load_messages", content, f"{script_name} missing loader import")
    ...
    def test_scripts_have_global_variables(self):
        ...
        for script_name in self.integrated_scripts:
            with self.subTest(script=script_name):
                ...
                self.assertIn("USER_LANG = ", content, f"{script_name} missing USER_LANG global variable")
                self.assertIn("messages = ", content, f"{script_name} missing messages global variable")
    ...
    def test_scripts_have_get_message_method(self):
        ...
        for script_name in self.integrated_scripts:
            with self.subTest(script=script_name):
                ...
                self.assertIn("def get_message(self, key", content, f"{script_name} missing get_message method")
    ...
    def test_scripts_have_language_initialization(self):
        ...
        for script_name in self.integrated_scripts:
            with self.subTest(script=script_name):
                ...
                self.assertIn("USER_LANG = get_language()", content, f"{script_name} missing language initialization")
                self.assertIn("messages = load_messages(", content, f"{script_name} missing message loading")
    ...
    def test_message_files_exist(self):
        ...
        for script_name in self.integrated_scripts:
            with self.subTest(script=script_name):
                ...
                self.assertTrue(os.path.exists(message_file), f"Message file {base_name}.json not found")
                ...
                if os.path.exists(message_file):
                    with open(message_file, "r", encoding="utf-8") as f:
                        try:
                            ...
                            self.assertIsInstance(messages, dict, f"{base_name}.json should contain a dictionary")
    ...
    def test_get_message_method_functionality(self):
        ...
        self.assertEqual(test_instance.get_message("formatted_key", "test"), "Message with test parameter")

        # Test multiple parameters
        self.assertEqual(
            test_instance.get_message("multi_param", "first", "second"), "Message with first and second parameters"
        )
    ...
    def test_backward_compatibility_main_execution(self):
        ...
        for script_name in self.integrated_scripts:
            with self.subTest(script=script_name):
                ...
                self.assertIn('if __name__ == "__main__":', content, f"{script_name} missing main execution guard")

    def test_message_key_usage_consistency(self):
        """Test that scripts use consistent message key patterns"""
        common_patterns = ["title", "separator", "debug.", "status.", "errors.", "prompts."]
            ...
            with self.subTest(script=script_name):
                ...
                if os.path.exists(message_file):
                    ...
                    has_common_pattern = any(
                        any(
                            key.startswith(pattern.rstrip(".")) or pattern.rstrip(".") in key
                            for key in self._flatten_dict(messages)
                        )
                        for pattern in common_patterns
                    )

                    self.assertTrue(has_common_pattern, f"{base_name}.json should contain common message patterns")
    ...
    def test_environment_variable_integration(self):
        ...
        for script_name in self.integrated_scripts:
            with self.subTest(script=script_name):
                ...
                self.assertIn("get_language()", content, f"{script_name} should call get_language() function")
