...
class TestBackwardCompatibility(unittest.TestCase):
        ...
        for script_name in self.integrated_scripts:
            with self.subTest(script=script_name):
                ...
                self.assertIn('if __name__ == "__main__":', content,
                            f"{script_name} should preserve main execution guard")
        ...
        for script_name in self.integrated_scripts:
            with self.subTest(script=script_name):
                ...
                self.assertIn("import boto3", content,
                            f"{script_name} should preserve boto3 import")
                
                # Check for client initialization patterns
                has_client_init = ("boto3.client" in content or 
                                 "self.iot_client" in content or
                                 "self.sts_client" in content)
                
                self.assertTrue(has_client_init,
                              f"{script_name} should preserve AWS client initialization")
    ...
    def test_scripts_preserve_error_handling(self):
        ...
        for script_name in self.integrated_scripts:
            with self.subTest(script=script_name):
                ...
                self.assertIn("from botocore.exceptions import ClientError", content,
                            f"{script_name} should preserve ClientError import")
                
                # Check for try-except blocks
                self.assertIn("try:", content,
                            f"{script_name} should preserve error handling")
    ...
    def test_scripts_preserve_colorama_usage(self):
        ...
        for script_name in self.integrated_scripts:
            with self.subTest(script=script_name):
                ...
                self.assertIn("from colorama import", content,
                            f"{script_name} should preserve colorama import")
                self.assertIn("init()", content,
                            f"{script_name} should preserve colorama initialization")
    ...
    def test_scripts_preserve_threading_patterns(self):
        ...
        for script_name in threading_scripts:
            with self.subTest(script=script_name):
                ...
                has_threading = ("from concurrent.futures import" in content or
                               "from threading import" in content)
                
                self.assertTrue(has_threading,
                              f"{script_name} should preserve threading imports")
        ...
        for script_name in self.integrated_scripts:
            with self.subTest(script=script_name):
                ...
                self.assertTrue(content.startswith('#!/usr/bin/env python3'),
                              f"{script_name} should preserve shebang line")
    ...
    def test_scripts_preserve_debug_mode_functionality(self):
        ...
        for script_name in self.integrated_scripts:
            with self.subTest(script=script_name):
                ...
                has_debug = ("debug_mode" in content or 
                           "debug" in content.lower())
                
                self.assertTrue(has_debug,
                              f"{script_name} should preserve debug functionality")
    ...
    def test_message_formatting_preserves_special_characters(self):
        ...
        self.assertEqual(test_instance.get_message("special_chars"), 
    ...
    def test_scripts_maintain_exit_codes(self):
        ...
        for script_name in self.integrated_scripts:
            with self.subTest(script=script_name):
                ...
                has_exit_handling = ("exit(" in content or 
                                   "sys.exit(" in content or
                                   "return" in content)
                
                self.assertTrue(has_exit_handling,
                              f"{script_name} should preserve exit handling")
