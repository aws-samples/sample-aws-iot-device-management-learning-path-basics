...
def validate_navigation_consistency():
    ...
    for readme_file in readme_files:
        ...
        navigation_tables[readme_file] = {
            'content': table_content,
            'entries': parse_navigation_table(table_content)
        }
        ...
        for filename, data in navigation_tables.items():
            ...
            for i, (ref_entry, entry) in enumerate(zip(reference_entries, entries)):
                ...
                if entry['link_url'] != ref_entry['link_url']:
                    errors.append(f"❌ {filename}: Link URL mismatch at position {i+1} ({entry['link_url']} vs {ref_entry['link_url']})")
                
                # Check link text consistency
                if entry['link_text'] != ref_entry['link_text']:
                    errors.append(f"❌ {filename}: Link text mismatch at position {i+1} ({entry['link_text']} vs {ref_entry['link_text']})")
