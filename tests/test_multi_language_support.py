...
class TestMultiLanguageSupport(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.supported_languages = {
            "en": {"name": "English", "sample_text": "Hello World", "direction": "ltr", "encoding": "utf-8"},
            "es": {"name": "Spanish", "sample_text": "Hola Mundo", "direction": "ltr", "encoding": "utf-8"},
            "ja": {"name": "Japanese", "sample_text": "こんにちは世界", "direction": "ltr", "encoding": "utf-8"},
            "zh-CN": {"name": "Chinese (Simplified)", "sample_text": "你好世界", "direction": "ltr", "encoding": "utf-8"},
            "pt-BR": {"name": "Portuguese (Brazil)", "sample_text": "Olá Mundo", "direction": "ltr", "encoding": "utf-8"},
            "ko": {"name": "Korean", "sample_text": "안녕하세요 세계", "direction": "ltr", "encoding": "utf-8"},
        }
    ...
    def test_language_code_mapping_completeness(self):
        ...
        self.assertEqual(
            mapped_languages, expected_languages, "Language codes in LANGUAGE_CODES should match supported languages"
        )
    ...
    def test_create_sample_messages_for_all_languages(self):
        ...
        sample_messages = {
            ...
            "prompts": {"continue": "Do you want to continue?", "input": "Please enter a value:"},
        }
        ...
        localized_samples = {
            ...
            "es": {
                ...
                "prompts": {"continue": "¿Desea continuar?", "input": "Por favor ingrese un valor:"},
            },
    ...
    def test_language_directory_structure(self):
        ...
        for lang_code in self.supported_languages.keys():
            ...
            if not os.path.exists(lang_dir):
                ...
                self.assertTrue(os.path.isdir(os.path.dirname(lang_dir)), f"Parent directory for {lang_code} should be valid")
    ...
    def test_unicode_support_for_all_languages(self):
        ...
        unicode_test_messages = {
            "en": {"greeting": "Hello! 👋", "symbols": "✓ ✗ ★ ♥ ⚠ ℹ", "numbers": "1234567890"},
            "es": {"greeting": "¡Hola! 👋", "symbols": "✓ ✗ ★ ♥ ⚠ ℹ", "accents": "áéíóúñü ÁÉÍÓÚÑÜ"},
            "ja": {
                "greeting": "こんにちは！👋",
            ...
            "zh-CN": {"greeting": "你好！👋", "simplified": "简体中文 汉字", "numbers": "一二三四五六七八九十"},
            "pt-BR": {"greeting": "Olá! 👋", "accents": "ãõçáéíóú ÃÕÇÁÉÍÓÚ", "symbols": "✓ ✗ ★ ♥ ⚠ ℹ"},
            "ko": {"greeting": "안녕하세요! 👋", "hangul": "한글 문자 테스트", "symbols": "✓ ✗ ★ ♥ ⚠ ℹ"},
        }
                ...
                for lang_code, expected_messages in unicode_test_messages.items():
                    with self.subTest(language=lang_code):
                        ...
                        for key, expected_value in expected_messages.items():
                            self.assertEqual(
                                loaded_messages[key], expected_value, f"Unicode content mismatch for {lang_code}.{key}"
                            )
    ...
    def test_language_extensibility(self):
        ...
        new_language = "fr"
        new_messages = {"title": "Script d'Exemple", "welcome": "Bienvenue dans l'application", "goodbye": "Au revoir!"}
    ...
    def test_current_language_availability(self):
        ...
        print(
            f"Supported but not yet implemented: {', '.join(set(self.supported_languages.keys()) - set(available_languages))}"
        )
    ...
    def test_language_file_consistency(self):
        ...
        for lang_code in available_languages:
            ...
            self.assertTrue(os.path.isdir(lang_dir), f"Language directory {lang_code} should exist")
