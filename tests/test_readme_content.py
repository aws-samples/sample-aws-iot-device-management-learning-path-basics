...
def extract_sections(content):
    ...
    for line in lines:
        ...
        if header_match:
            ...
            # Start new section
            current_section = {"level": header_match.group(1), "title": header_match.group(2).strip()}
            current_content = []
        else:
...
def extract_code_blocks(content):
    ...
    return {"blocks": code_blocks, "inline": inline_code}
...
def extract_aws_services(content):
    ...
    return {"aws_services": found_services, "technical_terms": found_terms}
...
def normalize_section_title(title):
    ...
    if any(
        ...
        keyword in title_lower
        for keyword in ["target audience", "audiencia objetivo", "対象読者", "대상 사용자", "público-alvo", "目标受众"]
    ):
        ...
        keyword in title_lower
        for keyword in ["prerequisites", "prerrequisitos", "前提条件", "전제 조건", "pré-requisitos", "先决条件"]
    ):
        return "Prerequisites"
    elif any(
        keyword in title_lower
        for keyword in ["cost analysis", "análisis de costos", "コスト分析", "비용 분석", "análise de custos", "成本分析"]
    ):
        return "Cost Analysis"
    elif any(
        keyword in title_lower
        for keyword in ["quick start", "inicio rápido", "クイックスタート", "빠른 시작", "início rápido", "快速开始"]
    ):
        return "Quick Start"
    elif any(
        keyword in title_lower
        ...
        keyword in title_lower
        for keyword in ["internationalization", "internacionalización", "国際化", "국제화", "internacionalização", "国际化"]
    ):
        return "Internationalization Support"
    elif any(
        keyword in title_lower
        for keyword in ["usage examples", "ejemplos de uso", "使用例", "사용 예시", "exemplos de uso", "使用示例"]
    ):
        ...
        keyword in title_lower
        for keyword in ["documentation", "documentación", "ドキュメント", "문서", "documentação", "文档"]
    ):
...
def validate_content_structure():
    ...
    for filename, data in file_data.items():
        ...
        for i, (ref_section, section) in enumerate(zip(reference_sections, sections)):
            ...
            if section["level"] != ref_section["level"]:
                errors.append(
                    f"❌ {filename}: Section level mismatch at position {i+1} (level {section['level']} vs {ref_section['level']})"
                )
            ...
            if ref_normalized != section_normalized:
                ...
                if ref_normalized != "subsection" and section_normalized != "subsection":
                    ...
                    if ref_section["level"] <= 2:  # Only check main sections (# and ##)
                        errors.append(
                            f"❌ {filename}: Major section structure mismatch at position {i+1} ('{section_normalized}' vs '{ref_normalized}')"
                        )
    ...
    for filename, data in file_data.items():
        ...
        if len(code_blocks["blocks"]) != len(reference_code["blocks"]):
            errors.append(
                f"❌ {filename}: Code block count mismatch ({len(code_blocks['blocks'])} vs {len(reference_code['blocks'])})"
            )
