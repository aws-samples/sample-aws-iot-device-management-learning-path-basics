...
**主要対象者：** IoT開発者、ソリューションアーキテクト、AWS IoTデバイスフリートを扱うDevOpsエンジニアの方々

**前提条件：** 中級レベルのAWS知識、AWS IoT Coreの基礎、Pythonの基礎、コマンドライン使用経験があると良いでしょう

**学習レベル：** 大規模デバイス管理への実践的アプローチを含むアソシエイトレベルです

## 🎯 学習目標

- **デバイスライフサイクル管理**: 適切なthingタイプと属性を使ってIoTデバイスをプロビジョニングしてみましょう
- **フリート組織**: デバイス管理のための静的および動的thingグループを作成してみましょう
- **OTAアップデート**: Amazon S3統合を使ったAWS IoT Jobsでファームウェアアップデートを実装してみましょう
- **パッケージ管理**: 自動化されたシャドウアップデートで複数のファームウェアバージョンを扱ってみましょう
- **ジョブ実行**: ファームウェアアップデート中のリアルなデバイス動作をシミュレーションしてみましょう
- **バージョン管理**: デバイスを以前のファームウェアバージョンにロールバックしてみましょう
- **リモートコマンド**: AWS IoT Commandsを使ってデバイスへリアルタイムコマンドを送信してみましょう
- **一括登録**: 製造規模のプロビジョニングを使って数百または数千のデバイスを効率的に登録してみましょう
- **リソースクリーンアップ**: 不要なコストを避けるためにAWSリソースを適切に管理しましょう

## 📋 前提条件

- **AWSアカウント** - AWS IoT、Amazon S3、AWS Identity and Access Management（AWS Identity and Access Management (IAM)）の権限があるもの
- **AWS認証情報** - 設定済みのもの（`aws configure`、環境変数、またはAWS Identity and Access Management (IAM)ロール経由で設定できます）
- **Python 3.10+** - pip、boto3、colorama、requestsライブラリが必要です（requirements.txtファイルを確認してください）
- **Git** - リポジトリをクローンするために使います
...
**コスト管理：**
- ✅ クリーンアップスクリプトがすべてのリソースを削除してくれます
- ✅ 短期間のデモリソースです
- ✅ 設定可能なスケール（小さく始められます）
- ⚠️ **完了時にはクリーンアップスクリプトを実行しましょう**
...
**スクリプト機能**：
- **ネイティブAWS SDK**: より良いパフォーマンスと信頼性のためにboto3を使っています
- **多言語サポート**: 英語へのフォールバック付きインタラクティブ言語選択ができます
- **デバッグモード**: すべてのAWS API呼び出しとレスポンスを表示できます
- **並列処理**: デバッグモード以外では同時実行操作が可能です
- **レート制限**: 自動AWS APIスロットリング準拠しています
- **進捗追跡**: リアルタイムで操作ステータスを確認できます
- **リソースタグ付け**: 安全なクリーンアップのために自動でワークショップタグを付けます
- **設定可能な命名**: デバイス命名パターンをカスタマイズできます

### リソースタグ付け

すべてのワークショップスクリプトは、クリーンアップ時に安全に識別できるよう、作成されたリソースに自動的に`workshop=learning-aws-iot-dm-basics`タグを付けます。これにより、ワークショップで作成されたリソースのみが削除されるようになっています。
...
**プレフィックス要件**：
- 英数字、ハイフン、アンダースコア、コロンのみ使えます
- 最大20文字まで
- 連番は自動的にゼロパディングされます（001-999）

## 🌍 国際化サポート

すべてのスクリプトは自動言語検出とインタラクティブ選択で多言語サポートを提供しています。

**言語選択**：
- **インタラクティブ**: スクリプトが初回実行時に言語選択を促してくれます
- **環境変数**: `AWS_IOT_LANG=ja`を設定すれば言語選択をスキップできます
- **フォールバック**: 翻訳が不足している場合は自動的に英語にフォールバックします
...
**メッセージカテゴリ**：
- **UI要素**: タイトル、ヘッダー、区切り線など
- **ユーザープロンプト**: 入力要求や確認メッセージ
- **ステータスメッセージ**: 進捗更新や成功/失敗通知
- **エラーメッセージ**: 詳細なエラー説明とトラブルシューティング情報
...
**よくある問題**：
- **認証情報**: `aws configure`、環境変数、またはAWS Identity and Access Management (IAM)ロール経由でAWS認証情報を設定してください
- **権限**: AWS Identity and Access Management (IAM)ユーザーがAWS IoT、Amazon S3、AWS Identity and Access Management (IAM)権限を持っているか確認しましょう
- **レート制限**: スクリプトがインテリジェントスロットリングで自動処理してくれます
- **ネットワーク**: AWS APIへの接続を確認してください

**デバッグモード**: 詳細なトラブルシューティングのために任意のスクリプトで有効化できます
```bash
...
**継続的な料金を避けるため、完了時には必ずクリーンアップを実行しましょう：**
```bash
...
クリーンアップスクリプトは、ワークショップリソースのみが削除されるよう、複数の識別方法を使っています：

1. **タグベース識別**（プライマリ）：`workshop=learning-aws-iot-dm-basics`タグを確認します
2. **命名パターンマッチング**（セカンダリ）：既知のワークショップ命名規則と一致させます
3. **関連付けベース**（ターシャリ）：ワークショップリソースに添付されたリソースを識別します
...
**安全機能**：
- ワークショップ以外のリソースは自動的にスキップされます
- 詳細なサマリーで削除およびスキップされたリソースを表示します
- デバッグモードで各リソースの識別方法を表示できます
- ドライランモードで実際の削除前にプレビューできます
...
**翻訳ガイドライン**：
- **フォーマットの保持**: 絵文字、色、特殊文字を保持しましょう
- **プレースホルダーの維持**: 動的コンテンツ用の`{}`プレースホルダーを保持しましょう
- **技術用語**: AWSサービス名は英語のままにしておきます
- **文化的適応**: 例と参照を適切に適応させましょう
- **一貫性**: すべてのファイル間で一貫した用語を使いましょう
