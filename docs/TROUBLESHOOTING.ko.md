...
이 가이드는 환경 설정 문제를 다루고 있어요. 스크립트별 문제가 있다면 스크립트 실행 시 디버그 모드를 활성화해 보세요 - 상황에 맞는 오류 메시지와 안내를 제공해 드립니다.
...
**해결책**: AWS IAM 사용자/역할에 필요한 권한이 있는지 확인해 보세요:
...
**참고**: 프로덕션 환경의 경우 최소 권한 원칙을 따르고 리소스를 적절히 제한하는 것이 좋아요.
...
스크립트 실행 중 문제가 발생하면 이렇게 해보세요:

1. **디버그 모드 활성화** - 자세한 API 호출 및 응답을 보여드려요
   ```
   🔧 Enable debug mode? [y/N]: y
   ```

2. **오류 메시지 읽기** - 스크립트가 상황에 맞는 안내를 제공해 드려요

3. **교육적 일시 정지 확인** - 개념과 요구 사항을 설명해 드려요

4. **전제 조건 확인** - 대부분의 스크립트는 먼저 `provision_script.py`를 실행해야 해요
